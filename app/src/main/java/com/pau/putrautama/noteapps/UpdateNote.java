...
public class UpdateNote extends AppCompatActivity {
    ...
    private void updateNoteToFirebase(){
        ...
        if(!TextUtils.isEmpty(currentTitle)&& !TextUtils.isEmpty(currentContent)){
            mDatabaseReference.child(noteId).child("title").setValue(currentTitle);
            mDatabaseReference.child(noteId).child("content").setValue(currentContent);
            mDatabaseReference.child(noteId).child("dateCreated").setValue(dateCreated);
            mDatabaseReference.child(noteId).child("dateModified").setValue(dateModified);
        }else {
            Toast.makeText(this,"Harap mengisi semua",Toast.LENGTH_SHORT).show();
        }
    ...
    private void deleteNoteFromFirebase() {
        Query noteQuerry = mDatabaseReference.child(noteId).equalTo(noteId);
